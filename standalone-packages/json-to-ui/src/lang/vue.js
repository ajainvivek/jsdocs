    ...
    if (!/src/.test(match)) {
        if (block === 'template') {
            if (!split[1]) {
                ...
                let code = prettier.format(split[2], { parser, plugins });
...
const vue = function (template, options = {}) {
    ...
    const script = constructScript({ view: template.name, meta: template.meta, data: template.data, actions: template.actions, options });
    let fileContent = '';
