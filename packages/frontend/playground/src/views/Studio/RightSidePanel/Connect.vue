<template>
	<div class="connect-block-wrapper">
		<Button class="pu-btn" size="large" @click="showUIKit">
			<Icon type="md-cube" class="pu-icon" size="16" />Add UI Kit
		</Button>
		<Button class="pu-btn" size="large">
			<Icon type="ios-link" class="pu-icon" size="16" />Add Plugin
		</Button>
		<Modal v-model="showModal" title="Add UI Kit" footer-hide>
			<i-input search placeholder="Search UI kit..." />
			<div class="card-list-wrapper">
				<Card class="card" v-for="dependency in dependencies" :key="dependency.id">
					<div @click="addNpmDependencyToPackage(dependency)">
						<img :src="dependency.icon" height="32">
						<h3>{{dependency.title}}</h3>
					</div>
					<div class="icon-wrapper" @click="removeNpmDependencyFromPackage(dependency)" v-bind:class="{ hide: !dependency.included }">
						<Tooltip theme="light" placement="left">
							<Icon type="ios-trash" size="24" class="icon" />
							<div slot="content">
								<p>Dependency is already added</p>
								<p><i>Click to remove dependency</i></p>
							</div>
						</Tooltip>
					</div>
				</Card>
...
export default class Connect extends Vue {
    ...
    private async removeNpmDependencyFromPackage(dependency) {
        ...
        this.$Notice.info({
            title: 'Package Dependency',
            desc: `Removing ${name} dependency from project`,
        });
    }
}
</script>
